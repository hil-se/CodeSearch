...
def test_model(test, dual_encoder, threshold=0.5):
    ...
    start_ts = time.time()
    dual_encoder.returnEncodedTensors(ts_feature, lang="CodeSearchNetFull")
    saveLinks(lang="CodeSearchNetFull")
    mrr_score, acc, time_taken = MRR(lang="CodeSearchNetFull", duplicates=True)
    result = {}
    print("Test:\nTrained:")
    result["Accuracy"] = acc
