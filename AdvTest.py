...
def test_model(test, dual_encoder, threshold=0.5):
    ...
    start_ts = time.time()
    dual_encoder.returnEncodedTensors(ts_feature, lang="AdvTest")
    saveLinks(lang="AdvTest")
    mrr_score, acc, time_taken = MRR(lang="AdvTest", duplicates=True)
    result = {}
    print("Test:\nTrained:")
    result["Accuracy"] = acc
...
def allLanguageExperiment(embedding="cbow",
    ...
    result = test_model(test, dual_encoder, threshold=0.5, duplicates=duplicates)
